...
export default function App() {
  ...
  return (
    <div className="min-h-dvh bg-slate-50 dark:bg-[#09090b] text-slate-800 dark:text-slate-200 p-4 md:p-6 flex flex-col font-sans selection:bg-indigo-500/30 selection:text-indigo-900 dark:selection:text-indigo-200 overflow-x-hidden transition-colors duration-300">
      <ScrollProgress />
      <header className="flex justify-between items-center gap-2 sm:gap-4 mb-6 max-w-7xl mx-auto w-full shrink-0">
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0">
          ...
          <div className="flex items-center gap-1 bg-slate-200 dark:bg-zinc-800/50 rounded-full p-1 border border-slate-300 dark:border-zinc-700/50 transition-colors duration-300">
            <button
              onClick={() => setLang('fr')}
              className={`px-2.5 sm:px-3 py-1 rounded-full transition-all text-xs font-bold ${lang === 'fr' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
            >
              FR
            </button>
            <button
              onClick={() => setLang('en')}
              className={`px-2.5 sm:px-3 py-1 rounded-full transition-all text-xs font-bold ${lang === 'en' ? 'bg-indigo-500 text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}
            >
              EN
            </button>
          </div>
